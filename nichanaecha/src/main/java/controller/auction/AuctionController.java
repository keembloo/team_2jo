...
public class AuctionController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		CarDto carDto = new CarDto(
				Integer.parseInt(fileList.get(0).getString()),		//cno 호출(0)
				fileList.get(1).getString(),						//제조사(1)
				fileList.get(2).getString(),						//차량번호(2)
				fileList.get(3).getString(),						//차량종류(3)
				Integer.parseInt(fileList.get(4).getString()),		//배기량(4)
				fileList.get(5).getString(),						//연료(5)
				fileList.get(6).getString(),						//차량명(6)
				fileList.get(7).getString(),						//제조년월(7)
				Integer.parseInt(fileList.get(8).getString()),		//주행거리(8)
				fileList.get(9).getString(),						//위도(9)
				fileList.get(10).getString(),						//경도(10)
				cmo,
				imgList);
		...
		// Dto 를 Dao 처리
		boolean result = AuctionDao.getInstence().register(CarDto);
